...
const createFields2 = async (fields, connection, orgId, module, idPerfil, userId) => {
    ...
    for (const field of fields) {
        try {
            const { name, type, id = null, position = null, sort_order = null, related_module = null, field_type = null, options = null } = field;
            ...
            if (searchField.length === 0) {
                ...
                INSERT INTO fields (name, api_name, type, field_type, related_module, related_id, module)
                VALUES (?, ?, ?, ?, ?, ?, ?);`,
                    [name, uniqueApiName, type, field_type, related_module, related_id, module]);
                idField = result.insertId;
                console.log("INSERT: ", result);
            } else {
                const [result] = await connection.execute(`
                UPDATE fields SET name = ?, api_name = ?, type = ?, field_type = ?, related_module = ?, related_id = ?, module = ? WHERE id = ?;`,
                    [name, apiName, type, field_type, related_module, related_id, module, searchField[0].id]);
            ...
            if (related_module != null) {
                const [searchRelatedModule] = await connection.execute(
                    'SELECT id FROM modulos_relacionados WHERE related_module = ? and related_id = ? and module_name = ? and api_name = ?;', 
                    [related_module, related_id, module, apiName]
                );
                if (searchRelatedModule.length === 0) {
                    await connection.execute(
                        'INSERT INTO modulos_relacionados (related_module, related_id, module_name, module_id, api_name) VALUES (?, ?, ?, ?, ?);', 
                        [related_module, related_id, module, userId, apiName]
                    );
                } else {
                    await connection.execute(`
